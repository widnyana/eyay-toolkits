...
---
Extract (if target is existing code) or sketch (if target is a task) the call graph for: $ARGUMENTS
...
If the full spec is needed, use the `graph-protocol` skill (notation) and the
`design-method` skill (the method) shipped in this plugin.
